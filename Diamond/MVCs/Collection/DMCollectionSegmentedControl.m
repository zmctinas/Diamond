...
@interface DMScrollView : UIScrollView
@end

@interface  DMCollectionSegmentedControl()
...
@property (nonatomic, strong) DMScrollView *scrollView;
...
@implementation DMCollectionSegmentedControl
...
- (void)commonInit {
    self.scrollView = [[DMScrollView alloc] init];
    ...
    if (self.type == HMSegmentedControlTypeText) {
        [self.sectionTitles enumerateObjectsUsingBlock:^(id titleString, NSUInteger idx, BOOL *stop) {
            ...
            if (self.selectedSegmentIndex == idx) {
                ...
                titleLayer.fontSize = self.font.pointSize + 2.0;
                titleLayer.frame = CGRectMake(rect.origin.x - 5, rect.origin.y - 3, rect.size.width + 10, rect.size.height);
            } else {
