...
- (void)asyncPlayingWithPath:(NSString *)aFilePath
    ...
    if (![fileManager fileExistsAtPath:wavFilePath]) {
        ...
        if (!covertRet) {
            if (completon) {
                completon([NSError errorWithDomain:@"文件转换失败"
...
- (void)asyncStartRecordingWithFileName:(NSString *)fileName
    ...
    if ([self isRecording]) {
        if (completion) {
            error = [NSError errorWithDomain:@"音量不足"
    ...
    if (!fileName || [fileName length] == 0) {
        error = [NSError errorWithDomain:@"文件不存在"
...
-(void)asyncStopRecordingWithCompletion:(void(^)(NSString *recordPath,
    ...
    if(![self isRecording]){
        if (completion) {
            error = [NSError errorWithDomain:@"录音尚未开始"
    ...
    if([_recorderEndDate timeIntervalSinceDate:_recorderStartDate] < [EMCDDeviceManager recordMinDuration]){
        if (completion) {
            error = [NSError errorWithDomain:@"录音时长太短"
...
-(NSError *)setupAudioSessionCategory:(EMAudioSession)session
    ...
    switch (session) {
        case EM_AUDIOPLAYER:
            // 设置播放category
            audioSessionCategory = [SettingEntiy sharedInstance].speakerSwitch ? AVAudioSessionCategoryPlayback : AVAudioSessionCategoryPlayAndRecord;
            break;
    ...
    if (isNeedActive) {
        BOOL success = [audioSession setActive:isActive
                                   withOptions:AVAudioSessionSetActiveOptionNotifyOthersOnDeactivation
                                         error:&error];
        if(!success || error){
            error = [NSError errorWithDomain:@"初始化播放器失败"
